...
void SkipToPCMAudioData(FILE* fpwave, int *nChannels, int *nBitsPerSample)
{
	RIFFHEADER riff;
	FMTBLOCK fmt;
	XCHUNKHEADER chunk;
	...
	if ( chunk.nChunkSize>16 )
	{
        ...
        *nBitsPerSample = wfx.nBitsPerSample;
	}
	else
	{
        ...
        *nChannels = fmt.wf.nChannels;
        *nBitsPerSample = fmt.wf.nBitsPerSample;
	}
...
void SkipWAVHeader(char* buf, int *nChannels, int *nBitsPerSample)
{
	RIFFHEADER riff;
	FMTBLOCK fmt;
	XCHUNKHEADER chunk;
...
NSData *SkipAndroid3GP(NSData *data) {
    u32 size = 0;
    u32 type =0;
    u32 boxSize =0;
...
int SkipCAFHeader(FILE* fpcaf) {

    CAFFileHeader  fHeader;
    fread(&fHeader, 1, sizeof(CAFFileHeader), fpcaf);
    if (0x66666163 != fHeader.mFileType) {
        return 0;
    }
    
    //desc free data
    unsigned int magics[3] = {0x63736564, 0x65657266, 0x61746164};
    for (int i = 0; i < 3; ++i) {
        unsigned int chunkType;
        fread(&chunkType, 1, sizeof(chunkType), fpcaf);
        
        if (magics[i]!= chunkType) {
            return 0;
        }
        ...
        char b[8];
        fread(b, 1, 8, fpcaf);
        chunkSize = readSint64(b);
//        fread(&chunkSize, 1, sizeof(chunkSize), fpcaf);
        if (chunkSize <= 0) {
            return 0;
        }
        if (i == 2) {
		...
		fseek(fpcaf, chunkSize, SEEK_CUR);
...
int ReadAudioPCMFrame(short speech[], char *data, int nChannels, int nBitsPerSample)
{
	int nRead = 0;
	int x = 0, y=0;
//	unsigned short ush1=0, ush2=0, ush=0;
	...
	{
//		nRead = fread(pcmFrame_8b1, (nBitsPerSample/8), PCM_FRAME_SIZE*nChannels, fpwave);
        memcpy(pcmFrame_8b1, data, nRead);
		for(x=0; x<PCM_FRAME_SIZE; x++)
		{
			speech[x] =(short)((short)pcmFrame_8b1[x] << 7);
		}
		...
		{
//			nRead = fread(pcmFrame_8b2, (nBitsPerSample/8), PCM_FRAME_SIZE*nChannels, fpwave);
            memcpy(pcmFrame_8b2, data, nRead);
			for( x=0, y=0; y<PCM_FRAME_SIZE; y++,x+=2 )
			{
				// 1 - 取两个声道之左声道
				speech[y] =(short)((short)pcmFrame_8b2[x+0] << 7);
			...
			{
//				nRead = fread(pcmFrame_16b1, (nBitsPerSample/8), PCM_FRAME_SIZE*nChannels, fpwave);
                memcpy(pcmFrame_16b1, data, nRead);
				for(x=0; x<PCM_FRAME_SIZE; x++)
				{
					speech[x] = (short)pcmFrame_16b1[x+0];
				}
				...
				{
//					nRead = fread(pcmFrame_16b2, (nBitsPerSample/8), PCM_FRAME_SIZE*nChannels, fpwave);
                    memcpy(pcmFrame_16b2, data, nRead);
					for( x=0, y=0; y<PCM_FRAME_SIZE; y++,x+=2 )
					{
						//speech[y] = (short)pcmFrame_16b2[x+0];
						speech[y] = (short)((int)((int)pcmFrame_16b2[x+0] + (int)pcmFrame_16b2[x+1])) >> 1;
...
int EncodeWAVEFileToAMRFile(const char* pchWAVEFilename, const char* pchAMRFileName, int nChannels, int nBitsPerSample)
{
	...
	SkipToPCMAudioData(fpwave, &nChannels, &nBitsPerSample);
...
NSData *EncodeWAVEToAMR(NSData *data, int nChannels, int nBitsPerSample) {
	...
	/* write magic number to indicate single channel AMR file storage format */
//	bytes = fwrite(AMR_MAGIC_NUMBER, sizeof(char), strlen(AMR_MAGIC_NUMBER), fpamr);
    [retData appendBytes:AMR_MAGIC_NUMBER length:strlen(AMR_MAGIC_NUMBER)];
	
	/* skip to pcm audio data*/
	SkipWAVHeader(buf, &nChannels, &nBitsPerSample);
	...
	{
		// read one pcm frame
		if (!ReadAudioPCMFrame(speech, buf, nChannels, nBitsPerSample)) break;
        ...
        [retData appendBytes:amrFrame length:byte_counter];
//		fwrite(amrFrame, sizeof (unsigned char), byte_counter, fpamr );
	}
...
int EncodeCAFFileToAMRFile(const char* pchCAFFilename, const char* pchAMRFileName) {
    FILE* fpcaf;
	FILE* fpamr;
	...
	/* skip to pcm audio data*/
	int offset = SkipCAFHeader(fpcaf);
    fseek(fpcaf, offset, SEEK_CUR);
	...
	{
		// read one pcm frame
		if (!ReadPCMFrame(speech, fpcaf, 1, 16)) break;
...
NSData *EncodeCAFToAMR(NSData *data) {
    return nil;
}
